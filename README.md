...
To verify that the installation was successful, we can run a minimal benchmark in ``fakeRemote`` mode which mimics a distributed setting by starting multiple processes on the local host.
```
./bench.sc fakeRemote --withClients 3 --remoteDir remote_logs --testing true
```

TODO describe what it runs and where results are etc.
